    ...
    $.validationEngineLanguage = {
        newLang: function(){
            $.validationEngineLanguage.allRules = {
				...
				"usernameCheck": {
                    "regex": "none",
                    "alertText": "* You cannot use your username within your password"
                },		
                "creditCard": {
                    "regex": "none",
				...
				"studentNum": {
                    "regex": /^[\-\+]?\d{0,8}$/,
                    "alertText": "* Must be a valid student ID (max 8 digits long)"
                },
				"alphaOnly": {
                    "regex": /^[a-zA-Z0-9]*$/,
                    "alertText": "* Must be alphanumeric characters only"
                },
				"numberNoStart": {
                    "regex": /^[^0-9]+.*$/,
                    "alertText": "* Passwords cannot start with a number"
                },	
				"numeralsCount": {
                    "regex": /^\D*(?:\d\D*){1,13}$/,
                    "alertText": "* Passwords must have a minimum of 1 number, and a maximum of 13"
                },	
				"nonASCII": {
                    "regex": /^[\x20-\x7E]+$/,
                    "alertText": "* Passwords cannot contain invalid characters"
                },		
                "number": {
...
(function($) {
    ...
    var methods = {
            ...
            for (var i = 0; i < rules.length; i++) {
                ...
                switch (rules[i]) {
                    ...
                    case "equals":
                        ...
                        break;
					case "usernameCheck":
                        errorMsg = methods._usernameCheck(field, rules, i, options);
                        break;
                    case "funcCall":
                ...
                if (errorMsg !== undefined) {
                    promptText += errorMsg + "<span class='errorMsgSpace'></span>";
		...
		/**
         * Check usernameCheck not in pass
         *
         * @param {jqObject} field
         * @param {Array[String]} rules
         * @param {int} i rules index
         ...
         * @return an error string if validation failed
         */
		_usernameCheck: function(field, rules, i, options) {
            var checkVals = rules[i + 1];
			var re = new RegExp( '^.*'+checkVals+'.*$', "ig" );

            if (!field.val().search(re)){
				console.log("match");	
				return options.allrules.usernameCheck.alertText;
			}   
        },
        /**
        ...
        */
        ...
        _buildPrompt: function(field, promptText, type, ajaxed, options) {
            ...
            switch (type) {
                case "change":
                    tabType = "bubbleNotifyTab change";
                    break;
                case "update":
					tabType = "bubbleNotifyTab update";
                    break;
				case "empty":
					tabType = "bubbleNotifyTab empty";
                    break;
				case "edit":
					tabType = "bubbleNotifyTab edit";
                    break;
                default:
                    /* it has error  */
					tabType = "bubbleNotifyTab error";
			...
			$(prompt).prepend("<div class='"+tabType+"'></div>");
            ...
            if (options.showArrow) {
                ...
                switch (positionType) {
                    ...
                    case "bottomRight":
                        ...
                        arrow.addClass("formErrorArrowBottom").html('<div class="innerArrowBottom"></div>');
                        break;
                    case "topLeft":
                    case "topRight":
                        arrow.html('<div class="innerArrowTop"></div>');
                        prompt.append(arrow);
                        break;
                }
            }
